...
export default function Home() {
  ...
  // CHARGEMENT DES POLICES (Crucial pour le design)
  ...
  const handleDeepLink = (event: { url: string }) => {
      console.log("Lien reçu :", event.url);
      ...
      const interval = setInterval(() => {
        ...
        if (i >= fullText.length) {
          ...
          Animated.timing(lineAnim, {
            toValue: isMobile ? vw * 0.8 : vw * 0.4,
            duration: 800,
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      let i = 0;
      const interval = setInterval(() => {
        setLoggedInDisplayedText(fullText.substring(0, i + 1));
        i++;
  ...
  // NAVIGATION (Rendu prioritaire)
  if (authMode === 'reset') {
    return <ResetPassword token={resetToken} onBack={() => setAuthMode('welcome')} onSuccess={() => setAuthMode('login')} />;
  }
  ...
  if (isLoggedIn) {
    const bgColor = colorCycleAnim.interpolate({
      inputRange: [0, 1, 2],
      outputRange: ['#271258', '#133687', '#271248']
    });
    ...
    return (
      <Animated.View style={[styles.loggedInContainer, { backgroundColor: bgColor }]}>
        <ScrollView contentContainerStyle={{ alignItems: 'center', paddingBottom: 50 }}>
          ...
          <View style={[styles.blackCardsContainer, isMobile && { flexDirection: 'column', height: 'auto', gap: 40 }]}>
            <TouchableOpacity style={[styles.cardWrapper, isMobile && { width: '90%' }]} onPress={() => setAuthMode('goals')}>
                <Image source={require('../../assets/objects/goals.jpg')} style={[styles.cardImage, { height: isMobile ? 200 : vh * 0.35 }]} resizeMode='cover' />
                <Text style={[styles.cardText, { color: '#5da9ec' }]}>100 000 GOALS{"\n"}CHALLENGE</Text>
            </TouchableOpacity>

            <TouchableOpacity style={[styles.cardWrapper, isMobile && { width: '90%' }]} onPress={() => setAuthMode("assists")}>
                <Image source={require('../../assets/objects/assists.png')} style={[styles.cardImage, { height: isMobile ? 200 : vh * 0.35 }]} resizeMode='cover' />
                <Text style={[styles.cardText, { color: '#b9e97b' }]}>10 000 ASSISTS{"\n"}CHALLENGE</Text>
            </TouchableOpacity>

            <TouchableOpacity style={[styles.cardWrapper, isMobile && { width: '90%' }]} onPress={() => setAuthMode('trophies')}>
                <Image source={require('../../assets/objects/trophies.png')} style={[styles.cardImage, { height: isMobile ? 200 : vh * 0.35 }]} resizeMode='cover' />
                <Text style={[styles.cardText, { color: '#ec5d9b' }]}>1 000 TROPHIES{"\n"}CHALLENGE</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={{ height: 60, backgroundColor: 'black', width: '100%', marginTop: 40 }} />
        </ScrollView>
      </Animated.View>
    );
  }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.centerContainer}>
        {authMode === 'welcome' && (
          <View style={styles.authWelcome}>
            ...
            <View style={[styles.buttonGroup, isMobile && { flexDirection: 'column' }]}>
              <TouchableOpacity style={styles.pillBtn} onPress={() => setAuthMode('login')}>
                  <Text style={styles.pillBtnText}>LOGIN</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.pillBtn} onPress={() => setAuthMode('register')}>
                  <Text style={styles.pillBtnText}>REGISTER</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  authWelcome: { alignItems: 'center', width: '100%' },
  titleText: { fontSize: 32, color: '#fff', fontFamily: 'VT323_400Regular', textAlign: 'center' },
  redLine: { height: 3, backgroundColor: '#f00', marginTop: 8 },
  buttonGroup: { flexDirection: 'row', marginTop: 40, gap: 20 },
  pillBtn: { borderWidth: 3, borderColor: '#fff', paddingVertical: 10, paddingHorizontal: 35, borderRadius: 20, minWidth: 160, alignItems: 'center' },
  ...
  blackCardsContainer: { width: '95%', backgroundColor: '#000', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: 20, borderRadius: 15 },
  cardWrapper: { alignItems: 'center', width: '30%' },
